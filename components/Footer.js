export default function Footer() {
    return (
        <footer className="flex justify-center px-4 text-text-white bg-blue-lightest" id="contact">
            <div className="container py-6">
                <h1 className="text-center text-lg font-Poppins-bold lg:text-2xl">
                    Contact Me <br /> alisequeira.la.240@gmail.com
            </h1>

                <hr className="h-px mt-6 bg-gray-700 border-none" />

                <div className="flex flex-col items-center justify-between mt-6 md:flex-row">
                    <div>

                    </div>
                    <div className="flex mt-4 md:m-0">
                        <div className="-mx-4 flex">
                            <a href="https://github.com/alisequeira" target="_blank" className="px-4 text-sm">
                                <img className="h-6 w-6 " src="data:image/svg+xml;base64," />
                            </a>
                            <a href="https://www.linkedin.com/in/ali-sequeira-obregon-758a49189/" target="_blank" className="px-4 text-sm">
                                <img className="h-6 w-6  text-md" src="data:image/svg+xml;base64," />
                            </a>
                            <a href="https://www.instagram.com/the_natural505/" target="_blank" className="px-4 text-sm">
                                <img className="h-6 w-6 " src="data:image/svg+xml;base64," />
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    )
}